  ...
  if(firstName.value && email.value && password.value) {
    if(!validateEmail.match(dog) || validateEmail.match(dot) || validatePassword.match(regWithUpperCase) || !sendYoubike.checked || (password.value != confirmPassword.value)) {
      ...
      if(!validateEmail.match(dog) || validateEmail.match(dot)) {
        formError.innerHTML = "Email Error"
      }
      console.log('email dog: ', !validateEmail.match(dog))
      console.log('email dot: ', validateEmail.match(dot))
      console.log('pass upperCase: ', validatePassword.match(regWithUpperCase))
      console.log('pass = c.pass: ', password.value != confirmPassword.value)
      console.log('checkbox: ',!sendYoubike.checked)
      // location.reload();
    } else {
